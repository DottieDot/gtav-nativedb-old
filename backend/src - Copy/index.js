...
const createNativeString = async(native, {useTypedefs, usePascalCase, exportHashes, exportComments}) => {
	...
	if (exportComments) {
		const fullNative = await Natives.getNative(native.hash)
...
const createHeaderFile = async(params) => {
	const namespaces = await Natives.getNamespaces()
	const res = []
	...
	for (const namespace of namespaces) {
		const {natives} = await Natives.getNamespace(namespace.name)
...
app.get('/namespace', async(req, res) => res.send(await Natives.getNamespaces()))

app.get('/namespace/:namespace', async(req, res) => res.send(await Natives.getNamespace(req.params.namespace)))

app.get('/native/:native', async(req, res) => res.send(await Natives.getNative(req.params.native)))
